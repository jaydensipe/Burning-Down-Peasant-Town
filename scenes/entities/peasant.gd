...
func _on_pursuing_state_physics_processing(delta: float) -> void:
	if (_current_burning_prop.being_serviced): state_chart.send_event("pursue_to_look")
...
func _on_watering_state_physics_processing(delta: float) -> void:
	#if !(_current_burning_entity.burnable_component.burning): state_chart.send_event("watering_to_looking")
