...
func _get_closest_burning_entity():
	...
	for entity: BurnableObjective in burning_entites:
		...
		if (pos < closest_distance):
			_current_burning_objective = entity
...
func _on_pursuing_state_physics_processing(delta: float) -> void:
	...
	if (_current_burning_objective.being_serviced):
		state_chart.send_event("pursue_to_look")
...
func _on_watering_state_physics_processing(delta: float) -> void:
	...
	_current_burning_objective.health_component.apply_health(1.0, _current_burning_objective.health_component.HEALTH_TYPES.HEALTH)
